...
function normalizePhone(phone) {
  ...
  return phone
    ...
    .replace(/^0+/, '0');    // keep only one leading zero
}
...
router.post('/verify-guest', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from(TABLE_NAME)
      // הוספנו guest_full_name כדי שתוכל לחזור ל-frontend
    ...
    // כאן מחזירים גם את השם וגם את מספר החדר – לשימוש ב-localStorage
    return res.json({
      success: true,
    ...
    console.error('Unexpected error in POST /api/reservations/verify-guest:', err);
    return res.status(500).json({
      success: false,
